...
from tools.build.homepage.config import HOMEPAGE_PET_ASSET_OUTPUT_NAME, MAX_YEAR, MIN_YEAR, REPO_ROOT, env_flag
...
def _render_index_html(title: str, data_file: str, detail_file: str = "") -> str:
    ...
    demo_banner = """      <div class="demo-banner">
        <div class="demo-banner-inner">
          <span class="demo-banner-icon" aria-hidden="true"><svg viewBox="0 0 24 24" width="22" height="22" fill="#FB7299"><path d="M17.813 4.653h.854c1.51.054 2.769.578 3.773 1.574 1.004.995 1.524 2.249 1.56 3.76v7.36c-.036 1.51-.556 2.769-1.56 3.773s-2.262 1.524-3.773 1.56H5.333c-1.51-.036-2.769-.556-3.773-1.56S.036 18.858 0 17.347v-7.36c.036-1.511.556-2.765 1.56-3.76 1.004-.996 2.262-1.52 3.773-1.574h.774l-1.174-1.12a1.234 1.234 0 0 1-.373-.906c0-.356.124-.658.373-.907l.027-.027c.267-.249.573-.373.92-.373.347 0 .653.124.92.373L9.653 4.44c.071.071.134.142.187.213h4.267a.836.836 0 0 1 .16-.213l2.853-2.747c.267-.249.573-.373.92-.373.347 0 .662.151.929.4.267.249.391.551.391.907 0 .355-.124.657-.373.906zM5.333 7.24c-.746.018-1.373.276-1.88.773-.506.498-.769 1.13-.786 1.894v7.52c.017.764.28 1.395.786 1.893.507.498 1.134.756 1.88.773h13.334c.746-.017 1.373-.275 1.88-.773.506-.498.769-1.129.786-1.893v-7.52c-.017-.765-.28-1.396-.786-1.894-.507-.497-1.134-.755-1.88-.773zM8 11.107c.373 0 .684.124.933.373.25.249.383.569.4.96v1.173c-.017.391-.15.711-.4.96-.249.25-.56.374-.933.374s-.684-.125-.933-.374c-.25-.249-.383-.569-.4-.96V12.44c0-.373.129-.689.386-.947.258-.257.574-.386.947-.386zm8 0c.373 0 .684.124.933.373.25.249.383.569.4.96v1.173c-.017.391-.15.711-.4.96-.249.25-.56.374-.933.374s-.684-.125-.933-.374c-.25-.249-.383-.569-.4-.96V12.44c.017-.391.15-.711.4-.96.249-.249.56-.373.933-.373Z"/></svg></span>
          <span class="demo-banner-text">欢迎B站用户投币 投币 三连：</span>
          <a class="demo-banner-link" href="https://www.bilibili.com/video/BV1u3LX66Eh7/" target="_blank" rel="noopener noreferrer">
            「我把2000年中国名人做成了动态地图，还能和李白聊天」
            <svg class="demo-banner-arrow" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M6 4l4 4-4 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
          </a>
        </div>
      </div>"""
...
<html lang="zh-CN">
  <head>
    ...
    <style>
      ...
      .home-precheck-card {{
        ...
        background: linear-gradient(180deg, rgba(255, 252, 245, 0.96), rgba(245, 237, 222, 0.76));
        box-shadow: 0 10px 24px rgba(80, 50, 20, 0.08);
      }}
      .home-precheck-card.is-error {{
        border-color: rgba(184, 69, 58, 0.24);
        background: linear-gradient(180deg, rgba(255, 247, 237, 0.98), rgba(254, 226, 226, 0.72));
      }}
      .home-precheck-title {{
        display: flex;
        align-items: center;
      ...
      .home-precheck-card.is-error .home-precheck-badge {{
        background: rgba(184, 69, 58, 0.13);
  ...
  <body class="min-h-screen">
      ...
      <div class="glass card theme-card home-search-card px-6 py-5 relative z-30 overflow-visible">
        ...
        <div class="relative">
          <div class="home-search-row">
            <input id="q" class="theme-input flex-1 px-4 py-2.5 rounded-xl" placeholder="输入你感兴趣的历史人物，如：李白、苏轼、张骞、郑和" />
          ...
          <div class="home-hot-people text-xs" aria-label="热门人物">
            <span class="theme-subtitle">热门人物：</span>
            <button class="home-hot-person-btn" type="button" data-hot-person="李白">李白</button>
            <button class="home-hot-person-btn" type="button" data-hot-person="苏轼">苏轼</button>
            <button class="home-hot-person-btn" type="button" data-hot-person="关羽">关羽</button>
          </div>
          <div id="precheckCard" class="home-precheck-card hidden"></div>
    ...
    <script>
      ...
      const $precheckCard = document.getElementById("precheckCard");
      ...
      const hidePrecheckCard = () => {{
        if (!$precheckCard) return;
        $precheckCard.classList.add("hidden");
        $precheckCard.classList.remove("is-error");
        $precheckCard.innerHTML = "";
      }};
      const renderPrecheckCard = (options) => {{
        if (!$precheckCard) return;
        const opt = options || {{}};
        const person = String(opt.person || "").trim();
        const title = String(opt.title || (person ? ("准备生成「" + person + "」") : "准备生成历史人物足迹")).trim();
        const badge = String(opt.badge || "可生成").trim();
        const meta = String(opt.meta || "已完成生成前检查，将进入资料检索、地点定位与质量校验流程。").trim();
        const isError = !!opt.error;
        const actions = Array.isArray(opt.actions) ? opt.actions : [];
        $precheckCard.classList.toggle("is-error", isError);
        $precheckCard.innerHTML =
          `<div class="home-precheck-title"><span>${{esc(title)}}</span><span class="home-precheck-badge">${{esc(badge)}}</span></div>` +
          `<div class="home-precheck-meta">${{esc(meta)}}</div>` +
          (actions.length ? `<div class="home-precheck-actions">${{actions.map((a) => `<button type="button" class="theme-button-secondary px-3 py-1.5 rounded-xl text-xs" data-precheck-action="${{esc(a.action || "")}}" data-precheck-person="${{esc(person)}}">${{esc(a.label || "继续")}}</button>`).join("")}}</div>` : "");
        $precheckCard.classList.remove("hidden");
      }};
      const renderGenerateFallback = (person, reason) => {{
        const name = String(person || "").trim();
        const detail = String(reason || "生成暂未完成").trim();
        renderPrecheckCard({{
          person: name,
          title: name ? ("暂时无法生成「" + name + "」完整足迹") : "暂时无法生成完整足迹",
          badge: "需要重试或换人",
          meta: detail + "。你可以换一个资料更充分的人物，或稍后重试。",
          error: true,
          actions: [
            {{ action: "retry", label: "重新尝试" }},
            {{ action: "sample", label: "试试李白" }},
            {{ action: "sample_sushi", label: "试试苏轼" }},
          ],
        }});
      }};
      const buildPrecheckMeta = (node, rawName) => {{
        const n = node || {{}};
        const years = formatYearRange(n.birth_year, n.death_year);
        const aliases = uniqStrings(Array.isArray(n.aliases) ? n.aliases : []).slice(0, 2).join(" / ");
        const tags = [];
        if (years) tags.push(years);
        if (aliases) tags.push("别名：" + aliases);
        if (n.has_story === false) tags.push("暂未收录页面，将生成新足迹");
        else if (n.person) tags.push("已有页面，可直接打开");
        if (!tags.length && rawName) tags.push("未在本地索引中命中，将尝试实时生成");
        return tags.join(" · ");
      }};
      ...
      const precheckPerson = async (personName) => {{
        const person = String(personName || "").trim();
        if (!person || (STATIC_SITE && !API_BASE)) return null;
        try {{
          const resp = await fetchWithTimeout(apiUrl("generate/precheck"), 8000, {{
            method: "POST",
      ...
      const renderPrecheckPayload = (payload, fallbackPerson) => {{
        if (!payload) return;
        const person = String(payload.normalized || fallbackPerson || "").trim();
        if (payload.allowed === false) {{
          renderPrecheckCard({{
            person,
            title: person ? ("暂不建议生成「" + person + "」") : "暂不建议生成",
            badge: "不建议生成",
            meta: String(payload.reason || "该输入暂不适合生成真实历史人物足迹。"),
            error: true,
            actions: [{{ action: "sample", label: "试试李白" }}, {{ action: "sample_sushi", label: "试试苏轼" }}],
          }});
          return;
        }}
        renderPrecheckCard({{
          person,
          title: person ? ("已识别为「" + person + "」") : "人物准入检查通过",
          badge: payload.cached ? "已收录，可秒开" : "可生成",
          meta: String(payload.reason || "准入检查通过，将进入生成流程。"),
        }});
      }};
      ...
      const openPerson = async (name) => {{
        ...
        if (!q) return;
        const precheck = await precheckPerson(q);
        if (precheck) {{
          renderPrecheckPayload(precheck, q);
          if (precheck.allowed === false) return;
          const url = String(precheck.url || "").trim();
          if (precheck.cached && url) {{
            try {{ localStorage.setItem("stellar_focus_person", String(precheck.normalized || q).trim()); }} catch (_) {{}}
            if (navigateToRelativeHtml(url.replace(/^\.\//, ""), {{ newTab: true }})) return;
          }}
          if (precheck.normalized && String(precheck.normalized).trim() !== q && $q) $q.value = String(precheck.normalized).trim();
        }}
        const targetName = String(precheck && precheck.normalized ? precheck.normalized : q).trim();
        const found = nodes.find((n) => n && String(n.person || "").trim() === targetName) || null;
        if (found && found.has_story === false) {{
          setGenStatus("「" + targetName + "」暂未收录人物页，正在尝试创建分析任务");
          ensurePendingPersonTab(targetName);
          ensurePersonGenerated(targetName);
          return;
        }}
      ...
      let personPageOpened = false;
        const tick = async () => {{
          ...
          if (st === "failed") {{
            ...
            setGenStatus(summary);
            renderGenerateFallback(person, summary);
            updatePixelProgressPanel({{
              visible: true,
              person,
              status: st,
      ...
      const ensurePersonGenerated = async (personName) => {{
        ...
        renderPrecheckCard({{
          person,
          title: "准备生成「" + person + "」的人物足迹",
          badge: "预检查通过",
          meta: "未找到本地页面，将进入资料检索、古今地名定位、内容整理与质量检查。",
        }});
        currentGeneratePerson = person;
        setGeneratingUI(true);
        ...
        try {{
          ...
          if (!data || data.ok !== true || !data.task_id) {{
            ...
            setGenStatus(msg);
              renderGenerateFallback(person, msg);
              updatePixelProgressPanel({{
              visible: true,
              person,
      ...
      const readInitialFocusPerson = () => {{
        let person = "";
        try {{
          const params = new URLSearchParams(window.location.search || "");
          person = String(params.get("person") || params.get("highlight") || "").trim();
        }} catch (_) {{}}
        if (!person) {{
          try {{
            person = String(window.localStorage && window.localStorage.getItem("stellar_focus_person") || "").trim();
            if (person && window.localStorage) window.localStorage.removeItem("stellar_focus_person");
          }} catch (_) {{}}
        }}
        return person;
      }};
      ...
      const onSearch = (ev) => {{
        ...
        const node = picked && picked.node ? picked.node : null;
        const name = node ? String(node.person || "").trim() : rawName;
        if (name) $q.value = name;
        hideSearchSuggest();
        renderPrecheckCard({{
          person: name || rawName,
          title: node ? ("已识别为「" + name + "」") : ("准备生成「" + rawName + "」"),
          badge: node && node.has_story !== false ? "已有页面" : "可尝试生成",
          meta: buildPrecheckMeta(node, rawName),
        }});
        if (node && node.has_story !== false) {{
          focusKnownPerson(name);
      ...
      document.querySelectorAll("[data-hot-person]").forEach((btn) => {{
        btn.addEventListener("click", () => {{
          const person = String(btn.getAttribute("data-hot-person") || "").trim();
          if (!person) return;
          if ($q) $q.value = person;
          onSearch();
        }});
      ...
      if ($precheckCard) {{
        $precheckCard.addEventListener("click", (e) => {{
          const btn = e.target && e.target.closest ? e.target.closest("[data-precheck-action]") : null;
          if (!btn) return;
          const action = String(btn.getAttribute("data-precheck-action") || "").trim();
          const person = String(btn.getAttribute("data-precheck-person") || "").trim();
          if (action === "retry" && person) {{
            if ($q) $q.value = person;
            ensurePendingPersonTab(person);
            ensurePersonGenerated(person);
          }} else if (action === "sample") {{
            if ($q) $q.value = "李白";
            onSearch();
          }} else if (action === "sample_sushi") {{
            if ($q) $q.value = "苏轼";
            onSearch();
          }}
        }});
      ...
      fetch(DATA_FILE).then((r) => r.json()).then((data) => {{
        ...
        prefillCoordsNoMap();
        syncSearchActionState();
        const initialFocusPerson = readInitialFocusPerson();
        if (initialFocusPerson) {{
          if ($q) $q.value = initialFocusPerson;
          window.setTimeout(() => focusKnownPerson(initialFocusPerson), 120);
        }}
        window.requestAnimationFrame(animate);
        if (DATA_DETAIL_FILE) {{
