"""首页 Markdown 内容提取 — 生卒年、出生地、关系、朝代、角色、作品。"""
...
from storymap.script.core.project_paths import data_corpus_file_path, story_person_names
...
# LLM 关系缓存
_PEOPLE_RELATIONS_LLM_CACHE: Optional[Dict[str, List[Dict[str, str]]]] = None


def _load_llm_relations_cache() -> Dict[str, List[Dict[str, str]]]:
    global _PEOPLE_RELATIONS_LLM_CACHE
    if _PEOPLE_RELATIONS_LLM_CACHE is not None:
        return _PEOPLE_RELATIONS_LLM_CACHE
    path = data_corpus_file_path("people_relations_llm.json")
    if not path.exists():
        _PEOPLE_RELATIONS_LLM_CACHE = {}
        return {}
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
        if isinstance(raw, dict):
            _PEOPLE_RELATIONS_LLM_CACHE = {str(k): v for k, v in raw.items() if isinstance(v, list)}
        else:
            _PEOPLE_RELATIONS_LLM_CACHE = {}
    except Exception:
        _PEOPLE_RELATIONS_LLM_CACHE = {}
...
def _strip_markdown_markers(value: str) -> str:
    """Remove residual ** / __ / ` markers from a plain-text field."""
...
def _extract_disambiguation(md_text: str) -> Tuple[List[str], str, List[str]]:
    ...
    # Reject values that are clearly other markdown fields
    if foreign and re.search(r"^\s*-\s*\*\*|时代|身份|出生|去世|领域|历史地位", foreign):
        foreign = ""
    d = (
...
def _match_role_band(part: str) -> Optional[Tuple[str, str, str]]:
    ...
    if not value:
        return None
    # Collect all candidate (band_key, band_label, keyword) that match, then pick
    # the keyword with the longest length to avoid substring false matches
    # (e.g. "文学家" inside "天文学家").
    candidates: List[Tuple[str, str, str]] = []
    for band_key, band_label, keywords in ROLE_BAND_SPECS:
        for keyword in keywords:
            if keyword in value or value in keyword:
                candidates.append((band_key, band_label, keyword))
    if not candidates:
        return None
...
def _load_work_summary_items(path: Path) -> Dict[str, Dict[str, Any]]:
    ...
    # 叠加 LLM 生成的 work_summary（优先级更高）
    llm_path = data_corpus_file_path("work_summary_llm.json")
    if llm_path.exists():
        try:
            llm_data = _read_json(llm_path)
            if isinstance(llm_data, dict):
                for key, value in llm_data.items():
                    ...
                    merged = dict(existing) if existing else {"title": title}
                    # LLM 字段覆盖
                    for field in ("one_liner", "genre", "era", "quote_policy", "quotes", "quote", "authors"):
                        llm_val = value.get(field)
                        if llm_val is not None and llm_val != "" and llm_val != []:
                            merged[field] = llm_val
                    if not merged.get("summary") or not str(merged.get("summary") or "").strip():
                        merged["summary"] = str(value.get("one_liner") or "").strip()
                    out[title] = merged
        except Exception:
            pass
