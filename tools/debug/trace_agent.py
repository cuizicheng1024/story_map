"""Trace Agent 管线调用全过程，输出为 Markdown 格式。"""
...
def main():
    ...
    result = generate_markdown_with_agents(llm=llm, person=person, max_revisions=2)
    ...
    if tool_traces:
        ...
        for i, t in enumerate(tool_traces):
            ...
            print(f"| {i+1} | `{tool_name}` | {agent_step} | {duration}ms | {status} |")
        print()
        for i, t in enumerate(tool_traces):
            tool_name = t.get("tool_name", "?")
            agent_step = t.get("agent_step", "")
            duration = t.get("duration_ms", "?")
            timed_out = t.get("timed_out", False)
            status = "❌ 超时" if timed_out else ("✅" if t.get("success", True) else "❌ 失败")
    ...
    if pm:
        ...
        for j, p in enumerate(pm):
            ...
            print(f"| {j+1} | {name} | {modern} | {lng} | {lat} | {source} |")
    else:
        print("(无)")
    print()
