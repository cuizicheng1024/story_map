"""LLM 客户端 — MiniMax/OpenAI/Anthropic 兼容接口。

从 registry.py 拆分而来，单独管理 LLM 通信层。
"""
...
class StoryAgentLLM:
    ...
    def __init__(
        ...
        """
        # Canonical env var names. Older/deprecated names are listed in _FALLBACK_ENV_MAP below.
        fallback_model = _first_env(
            "LLM_MODEL_ID", "MINIMAX_MODEL", "MODEL", "MIMO_MODEL",
        )
        fallback_key = _first_env(
            "MINIMAX_API_KEY", "MIMO_API_KEY", "API_KEY",
        )
        fallback_base = (
            _first_env("LLM_BASE_URL", "MINIMAX_BASE_URL", "MIMO_BASE_URL", "BASE_URL")
