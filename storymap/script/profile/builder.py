...
def _sort_profile_locations(loc_items: List[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    # 先按出现顺序解析每个 location 的时间区间。
    bounds: List[Tuple[Optional[int], Optional[int]]] = [
        _extract_location_time_bounds(item.get("time")) for item in loc_items
    ]

    def _kind(item: Dict[str, object]) -> str:
        if str(item.get("type") or "").strip().lower() == "birth":
            return "birth"
        if str(item.get("type") or "").strip().lower() == "death" or _looks_like_death_location(item):
            return "death"
        return "normal"

    kinds = [_kind(item) for item in loc_items]

    # 仅对"夹在两个已知年份之间、且本身没解析出年份"的 normal 行迹做插值，
    # 例如马可·波罗里的"杭州（行在）—— 在华期间（具体时间不详）"。
    # 这样既能避免它被甩到所有有年份的行迹之后、越过 death 节点，
    # 又不破坏"开头/末尾的无年份记录应保持后置"的既有行为
    # （如蒙古早期"约12世纪80年代"出现在所有已知年份之后）。
    ...
    for i, start in enumerate(propagated_start):
        if start is not None:
            continue
        if kinds[i] != "normal":
            # birth/death 维持原逻辑（birth 走 -1e9 fallback，death 走 1e9 fallback）。
            continue
...
def build_profile_data(
    ...
    *,
    ...
    birth_geo = parser_utils._pick_geocode_name(birth_modern or birth_geocode_loc or birth_loc)
    death_geo = parser_utils._pick_geocode_name(death_modern or death_geocode_loc or death_loc)
    birth_coord = None
    death_coord = None
    if birth_geocode_loc:
        birth_coord = (
            parser_utils._extract_inline_coord_pair(birth_text)
            or fuzzy_coord_lookup(coords_cache, [birth_geo, birth_modern, birth_geocode_loc, birth_loc])
        )
        if not birth_coord:
            birth_coord = lookup_coords_from_historical_index(birth_geo, birth_modern, birth_geocode_loc, birth_loc)
        if allow_geocode and (not birth_coord) and birth_geo:
            birth_coord = resolve_place_coord(birth_geo, None, birth_geocode_loc, birth_modern)
    if death_geocode_loc:
        death_coord = (
            parser_utils._extract_inline_coord_pair(death_text)
            or fuzzy_coord_lookup(coords_cache, [death_geo, death_modern, death_geocode_loc, death_loc])
        )
        if not death_coord:
            death_coord = lookup_coords_from_historical_index(death_geo, death_modern, death_geocode_loc, death_loc)
        if allow_geocode and (not death_coord) and death_geo:
            death_coord = resolve_place_coord(death_geo, None, death_geocode_loc, death_modern)
