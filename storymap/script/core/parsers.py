...
def _pick_geocode_name(text: str) -> str:
    ...
    if match:
        return _normalize_geocode_target(match.group(1))
    for sep in [" / ", "/", "或", "、", "，", ",", "；", ";"]:
        if sep in text:
            text = text.split(sep, 1)[0]
            break
    return _normalize_geocode_target(re.sub(r"[（(].*?[）)]", "", text))


_ADMIN_LEAF_RE = re.compile(
    r"[A-Za-z\u4e00-\u9fff·\-\s]{1,30}?(?:特别行政区|自治区|自治州|地区|盟|省|市|县|区|郡|府|道|都|京|镇|乡|村|岛|港|州(?!市))"
)


def _normalize_geocode_target(text: str) -> str:
    cleaned = str(text or "").strip()
    if not cleaned:
        return ""
    cleaned = re.sub(r"^(今|现|今称|现称)\s*", "", cleaned).strip()
    cleaned = re.sub(r"\s+", "", cleaned)
    matches = list(_ADMIN_LEAF_RE.finditer(cleaned))
    if matches:
        leaf = matches[-1].group(0).strip()
        if leaf:
            return leaf
    return cleaned
...
def _birthplace_has_alternate_place_note(text: str) -> bool:
    value = str(text or "").strip()
    if not value:
        return False
    return bool(
        re.search(
            r"(存疑|说法不一|一说|另说|另有传说)[^，。；（）()]*?(出自|生于|出生于|来自)[^，。；（）()]*?(省|市|县|区|州|郡|岛|城|国|京|镇|乡|村|海|山)",
            value,
        )
        or re.search(
            r"[（(][^）)]*(存疑|说法不一|一说|另说|另有传说)[^）)]*(出自|生于|出生于|来自)[^）)]*(省|市|县|区|州|郡|岛|城|国|京|镇|乡|村|海|山)[^）)]*[）)]",
...
def _historical_index_candidates() -> List[Path]:
    ...
    # 本文件位置：<repo>/storymap/script/core/parsers.py，parents[3]=<repo>
    repo_root = here.parents[3]
    candidates = [
        data_corpus_file_path("historical_places_index.jsonl", project_root=repo_root),
...
def _parse_date_location_details(text: str, keys: List[str]) -> tuple[str, str, str]:
    ...
    for p in parts:
        ...
        loc = cand
        break
    loc = _strip_common_birthplace_prefixes(loc.strip("。；; ")).strip("。；; ")
    loc = _strip_birthplace_date_ambiguity_text(loc).strip("。；; ")
    geocode_loc = loc
    if _birthplace_has_multiple_place_options(loc):
        geocode_loc = ""
    elif _birthplace_text_is_ambiguous(loc):
        geocode_loc = ""
...
def normalize_basic_info_birth_death_fields(md: str) -> str:
    if not isinstance(md, str) or not md.strip():
        return "" if md is None else str(md or "")
    lines = md.splitlines()
    in_profile = False
    in_basic = False
    updated: List[str] = []
    for line in lines:
        ...
        if in_profile and stripped.startswith("### "):
            ...
            updated.append(line)
            continue
        if in_profile and in_basic:
            m = re.match(r"(\s*-\s*\*\*(出生|去世)\*\*：\s*)(.+?)\s*$", line)
            if m:
    ...
    for line in lines:
        ...
        if table_started:
            ...
            if lat is None or lon is None:
                continue
            if raw_name:
                coords[raw_name] = (float(lat), float(lon))
            if name:
