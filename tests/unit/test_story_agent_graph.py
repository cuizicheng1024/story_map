...
import story_agent_state
import story_agent_tool_runner
...
def test_fetch_ancient_place_map_auto_queues_unresolved_place(tmp_path, monkeypatch):
    class FakeGeocodeService:
        ...
        @staticmethod
        def lookup_coords_from_historical_index(*_names: str):
            return None

        @staticmethod
        def resolve_place_coord(_place: str, _year=None, *_aliases: str):
            return None
...
def test_story_markdown_agent_uses_fallback_markdown_when_llm_budget_exhausted():
    ...
    assert "## 四、生平时间线" in result["markdown"]
    assert "| 701年 | 碎叶城 | 碎叶城 | 出生 |" in result["markdown"]
...
def test_story_markdown_agent_maps_timeline_places_even_when_places_list_is_empty():
    ...
    assert result["markdown"] == "# 李白\n"
    assert mapped == ["碎叶城", "当涂"]
    assert [item["query"] for item in result["state"]["place_maps"]] == ["碎叶城", "当涂"]
